...
class DecoderRNN(nn.Module):
    ...
    def _forward_train(self, encoder_outputs, problem_output, text_tgt):
        ...
        tgt_novar_id = torch.clamp(text_tgt, max=self.var_start-1)
        novar_embedding = self.embedding_tgt(tgt_novar_id)
        tgt_var_id = torch.clamp(text_tgt - self.var_start, min=0)
        var_embeddings = self.embedding_var.gather(
            dim=1, index=tgt_var_id.unsqueeze(2).repeat(1, 1, self.cfg.decoder_embedding_size)
        )
        choose_mask = (text_tgt < self.var_start).unsqueeze(2).repeat(1, 1, self.cfg.decoder_embedding_size)
        embedding_all = torch.where(choose_mask, novar_embedding, var_embeddings)
        embedding_all_ = self.em_dropout(embedding_all)
        embedding_weight_no_var = self.embedding_tgt(self.no_var_id.repeat(batch_size, 1))
        embedding_weight_all = torch.cat((embedding_weight_no_var, self.embedding_var), dim=1)
        embedding_weight_all_ = self.em_dropout(embedding_weight_all)

        for t in range(text_tgt.size(1) - 1):
            current_hiddens = self.em_dropout(rnn_hidden[-1].unsqueeze(1))
            attn_weights = self.attn(current_hiddens, encoder_outputs, self.src_mask)
            context = attn_weights.unsqueeze(1).bmm(encoder_outputs)
            rnn_output, rnn_hidden = self.gru(
                torch.cat((embedding_all_[:, t:t+1, :], context), 2), rnn_hidden
            )
            current_fusion_emb = torch.cat((rnn_output, context), 2)
            current_fusion_emb_ = self.em_dropout(current_fusion_emb)
            candi_score = self.score(current_fusion_emb_, embedding_weight_all_, self.candi_mask)
    ...
    def _forward_test(self, encoder_outputs, problem_output):
        ...
        for sample_id in range(batch_size):
            ...
            encoder_output = encoder_outputs[sample_id:sample_id+1].repeat(rem_size, 1, 1)
            src_mask = self.src_mask[sample_id:sample_id+1].repeat(rem_size, 1)
            embedding_var = self.embedding_var[sample_id:sample_id+1].repeat(rem_size, 1, 1)
            embedding_weight_no_var = self.embedding_tgt(self.no_var_id.repeat(rem_size, 1))
            embedding_weight_all = torch.cat((embedding_weight_no_var, embedding_var), dim=1)
            embedding_weight_all_ = self.em_dropout(embedding_weight_all)
            candi_mask = self.candi_mask[sample_id:sample_id+1].repeat(rem_size, 1)
            candi_exp_output = []
            candi_score_output = []
            
            for i in range(self.cfg.max_output_len):
                if i == 0:
                    input_token = torch.LongTensor([[self.sos_id]] * rem_size).cuda()
                    rnn_hidden = problem_output[:, sample_id:sample_id+1].repeat(1, rem_size, 1)
                    current_score = torch.FloatTensor([[0.0]] * rem_size).cuda()
                ...
                tgt_novar_id = torch.clamp(input_token, max=self.var_start-1)
                novar_embedding = self.embedding_tgt(tgt_novar_id)
                tgt_var_id = torch.clamp(input_token - self.var_start, min=0)
                var_embeddings = embedding_var[:rem_size].gather(
                    dim=1, index=tgt_var_id.unsqueeze(2).repeat(1, 1, self.cfg.decoder_embedding_size)
                )
                choose_mask = (input_token < self.var_start).unsqueeze(2).repeat(1, 1, self.cfg.decoder_embedding_size)
                embedding_all = torch.where(choose_mask, novar_embedding, var_embeddings)
                embedding_all_ = self.em_dropout(embedding_all)
                current_hiddens = self.em_dropout(rnn_hidden[-1].unsqueeze(1))
                attn_weights = self.attn(current_hiddens, encoder_output[:rem_size], src_mask[:rem_size])
                context = attn_weights.unsqueeze(1).bmm(encoder_output[:rem_size])
                rnn_output, rnn_hidden = self.gru(torch.cat((embedding_all_, context), 2), rnn_hidden)
                current_fusion_emb = torch.cat((rnn_output, context), 2)
                current_fusion_emb_ = self.em_dropout(current_fusion_emb)
                candi_score = self.score(current_fusion_emb_, embedding_weight_all_[:rem_size], candi_mask[:rem_size])

                new_score = F.log_softmax(candi_score, dim=1) if i == 0 else F.log_softmax(candi_score, dim=1) + current_score

                cand_tup_list = [(score, idx) for idx, score in enumerate(new_score.view(-1).tolist())]
                ...
                for score, idx in cand_tup_list[:self.cfg.beam_size]:
                    if idx != -1:
                        x = idx // candi_score.size(-1)
                        y = idx % candi_score.size(-1)
                        if y != self.eos_id:
                            token_list.append(y)
                            cand_list.append(x)
                            ...
                            candi_score_output.append(float(score))
